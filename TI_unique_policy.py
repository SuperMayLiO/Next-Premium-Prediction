...
def PreprocessClaim(num_rows = None, nan_as_category = False):
    ...
    df = df.join(claim_agg, how='left', on='Policy_Number')
...
def PrepreocessPolicy(num_rows = None, nan_as_category = False):
    ...
    exclude_list = ["Next_Premium", "Policy_Number", "Insured's_ID",'Prior_Policy_Number','Vehicle_identifier','Vehicle_Make_and_Model2','Coding_of_Vehicle_Branding_&_Type','Distribution_Channel','aassured_zip','ibirth','dbirth']
...
min_split_gain = 0.01
min_child_weight = 5
cv_result = lgb.cv(params, train_data, nfold=2, seed=1, stratified=True, verbose_eval =200, metrics=['mae'])
    

def lgb_eval(num_leaves, feature_fraction, bagging_fraction, max_depth, lambda_l1, lambda_l2, min_split_gain, min_child_weight):
    params = {'application':'binary','num_iterations':4000, 'learning_rate':0.05, 'early_stopping_round':100, 'metric':'mae'}
...
def bayes_parameter_opt_lgb(X, y, init_round=15, opt_round=25, n_folds=5, random_seed=6, n_estimators=10000, learning_rate=0.05, output_process=False):
    ...
    def lgb_eval(num_leaves, feature_fraction, bagging_fraction, max_depth, lambda_l1, lambda_l2, min_split_gain, min_child_weight, max_bin):
        params = {'application':'binary','num_iterations': n_estimators, 'learning_rate':learning_rate, 'early_stopping_round':100, 'metric':'mae'}
        params["num_leaves"] = int(round(num_leaves))
        params['feature_fraction'] = max(min(feature_fraction, 1), 0)
        params['bagging_fraction'] = max(min(bagging_fraction, 1), 0)

...
def PreprocessPolicy(num_rows = None, nan_as_category = False):
    ...
    policy_aggregations = {'Premium': ['min', 'max', 'mean', 'size', 'median', 'std', 'sum', 'nunique'],
                       ...
                       'Refund_Coverage_Deductible_if_applied': [ 'mean', 'size'],
                       'Partial_Refund_1st_Coverage_Deductible_if_applied': [ 'mean', 'size'],
                       'Partial_Refund_2nd_Coverage_Deductible_if_applied': [ 'mean', 'size'],
                       'Partial_Refund_3rd_Coverage_Deductible_if_applied': [ 'mean', 'size'],
                       'Partial_Refund_mean_Coverage_Deductible_if_applied': [ 'mean', 'size'],
                       'Amount_Refund_1st_Coverage_Deductible_if_applied': [ 'mean', 'size'],
                       'Amount_Refund_2nd_Coverage_Deductible_if_applied': [ 'mean', 'size'],
                       'Amount_Refund_3rd_Coverage_Deductible_if_applied': [ 'mean', 'size'],
                       'Amount_Refund_mean_Coverage_Deductible_if_applied': [ 'mean', 'size'],
                       'Paid_Coverage_Deductible_if_applied': [ 'mean', 'size'],
                       'Partial_Paid_1st_Coverage_Deductible_if_applied': [ 'mean', 'size'],
                       'Partial_Paid_2nd_Coverage_Deductible_if_applied': [ 'mean', 'size'],
                       'Partial_Paid_3rd_Coverage_Deductible_if_applied': [ 'mean', 'size'],
                       'Partial_Paid_mean_Coverage_Deductible_if_applied': [ 'mean', 'size'],
                       'Amount_Paid_1st_Coverage_Deductible_if_applied': [ 'mean', 'size'],
                       'Amount_Paid_2nd_Coverage_Deductible_if_applied': [ 'mean', 'size'],
                       'Amount_Paid_3rd_Coverage_Deductible_if_applied': [ 'mean', 'size'],
                       'Amount_Paid_mean_Coverage_Deductible_if_applied': [ 'mean', 'size'],
...
def PreprocessPolicy_small(num_rows = None, nan_as_category = False):
    # Read data and merge
    df = pd.read_csv('../data/training-set.csv', nrows= num_rows)
    test_df = pd.read_csv('../data/testing-set.csv', nrows= num_rows)
    try:
...
def kfold_lightgbm_step1(df, num_folds, stratified = False, debug= False):
    # Divide in training/validation and test data
    train_df = df[df['Next_Premium'].notnull()]
    test_df = df[df['Next_Premium'].isnull()]
    
    train_df['TARGET'] = np.where(train_df['Next_Premium'] > 0, 1, 0)
    train_df.drop('Next_Premium', axis=1)
    test_df['TARGET'] = test_df['Next_Premium']
    test_df.drop('Next_Premium', axis=1)
   ...
   # del df
    ...
    if stratified:
        ...
        folds = sklearn.model_selection.KFold(n_splits= num_folds, shuffle=True, random_state=47)
 ...
 'TARGET']]
    ...
    train_x, valid_x, train_y, valid_y = cross_validation.train_test_split(train_df[feats], train_df['TARGET'], test_size=test_size, random_state=seed)
...
#step1_train_idx = oof_preds_step1
#step1_valid_idx = np.array(sub_preds_step1)

def kfold_lightgbm_step2(df, num_folds, step1_train_idx, step1_valid_idx, stratified = False, debug= False):
    # Divide in training/validation and test data
    train_df = df[df['Next_Premium'].notnull()][step1_train_idx==1]
    test_df = df[df['Next_Premium'].isnull()][np.array(step1_valid_idx)==1]
 ...
 'Accident_Time']]
   ...
   # feats = [f for f in feats if f not in feature_importance_df[feature_importance_df.importance ==0].feature.tolist()]
    ...
    if not debug:
        ...
        test_df_submit['Next_Premium'][np.isnan(test_df_submit['Next_Premium'])] = 0 
...
oof_preds01, sub_preds01 = kfold_lightgbm_step1(df, num_folds)
oof_preds_step1 = oof_preds01
sub_preds_step1 = [1 if x>=0.5 else 0 for x in sub_preds01]

kfold_lightgbm_step2(df, num_folds, oof_preds_step1, sub_preds_step1)
